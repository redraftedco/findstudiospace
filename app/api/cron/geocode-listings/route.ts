...
// Portland neighborhood centroids — avoids a network call for the most common case
...
export async function GET(req: NextRequest) {
  ...
  for (const l of listings) {
    ...
    // Tier 2: neighborhood centroid (no network)
    if (lat === null && CENTROIDS[hood]) {
      ;[lat, lon] = CENTROIDS[hood]
      tier = 'neighborhood'
    }

    // Tier 3: Nominatim neighborhood query
