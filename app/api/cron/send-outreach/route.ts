 ...
 *
 * Daily 06:00 UTC — runs after enrich-targets (05:00).
 *
 ...
 *
 * Cap: 20 emails per run to stay well inside Resend's daily limit and
 * avoid appearing as a bulk sender. Adjust DAILY_SEND_CAP via env var.
 *
...
const SEND_DELAY_MS  = 150   // pause between Resend API calls
...
export async function GET(req: NextRequest) {
  ...
  // ── CAN-SPAM gate — throws if MAILING_ADDRESS not configured ─────────────
  try {
  ...
  for (const target of targets) {
    ...
    // Pace sends — avoid Resend burst rate limits
