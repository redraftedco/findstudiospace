...
const AMENITY_FILTERS: Record<string, AmenityMeta[]> = {
  'photo-studios': [
    ...
    },
    {
      key: 'natural_light',
      label: 'Natural Light',
      h1: 'Natural Light Photography Studios in Seattle',
      title: 'Natural Light Studios for Rent in Seattle, WA | FindStudioSpace',
      description: 'Seattle photography studios with skylights and north-facing windows. Natural light studio rentals for portrait, editorial, and lifestyle work.',
    },
    {
      key: 'green_screen',
      label: 'Green Screen',
      h1: 'Green Screen Studios in Seattle',
      title: 'Green Screen Studios for Rent in Seattle, WA | FindStudioSpace',
      description: 'Find Seattle studios with green screen / chroma-key setups for video production and composite photography.',
    },
    {
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const baseUrl = `https://www.findstudiospace.com/seattle/${category}`
  if (amenityMeta) {
    ...
    return {
      ...
      openGraph: { title: amenityMeta.title, description: amenityMeta.description },
    }
  }
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const itemListRows = listings.slice(0, 100)
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Seattle', item: 'https://www.findstudiospace.com/seattle' },
      amenityMeta
        ? { '@type': 'ListItem', position: 3, name: config.h1, item: `https://www.findstudiospace.com/seattle/${category}` }
      ...
      <main style={{ background: 'var(--paper)', color: 'var(--ink)' }} className="min-h-screen">
        <div className="mx-auto max-w-4xl px-6 py-10">
          <nav style={{ color: 'var(--stone)', fontFamily: 'var(--font-mono)' }} className="mb-6 text-xs">
            ...
            <Link href="/seattle" className="hover:underline">Seattle</Link>
            <span className="mx-2">→</span>
            {amenityMeta ? (
              <>
                <Link href={`/seattle/${category}`} className="hover:underline">{config.h1}</Link>
                <span className="mx-2">→</span>
            ...
            <div className="mb-10 flex flex-wrap gap-2">
              ...
              {availableAmenities.map((a) => {
                ...
                return (
                  <Link
                    key={a.key}
                    href={`/seattle/${category}?amenity=${a.key}`}
                    style={{
          ...
          {q && (
            <p style={{ color: 'var(--stone)', fontFamily: 'var(--font-mono)' }} className="mb-6 text-sm">
              ...
              {listings.length === 0 && (
                <> <Link href={`/seattle/${category}`} className="underline">Browse all spaces →</Link></>
              )}
            </p>
          )}
          ...
          {sponsoredListings.length > 0 && (
            <div style={{ marginBottom: '32px' }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--stone)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '12px' }}>
                Sponsored
              </p>
