 ...
 *   const { subject, html, text } = buildOutreachEmail('photo-v1', { ... })
 */
...
// PHOTO STUDIO — priority 90
// Audience: photographers, production companies, creative directors renting space
function photoTemplate(args: OutreachEmailArgs): OutreachEmail {
  ...
  const subject = `${name} — your photo studio listing on FindStudioSpace`
...
// MAKERSPACE / WORKSHOP — priority 75
// Audience: makerspace operators, fab labs, woodworking/metal shops
function makerTemplate(args: OutreachEmailArgs): OutreachEmail {
  ...
  const subject = `${name} — your workshop listing on FindStudioSpace`
...
// EVENT VENUE / BREWERY / ROOFTOP — priority 70
// Audience: venue managers, brewery event coordinators
function eventTemplate(args: OutreachEmailArgs): OutreachEmail {
  ...
  const subject = `${name} — your event space listing on FindStudioSpace`
...
// PODCAST / RECORDING STUDIO — priority 70
// Audience: podcast studio operators, recording engineers with rental booths
function podcastTemplate(args: OutreachEmailArgs): OutreachEmail {
  ...
  const subject = `${name} — your podcast studio listing on FindStudioSpace`
...
// GENERIC — art studios, office/cowork, creative space catch-all
function genericTemplate(args: OutreachEmailArgs): OutreachEmail {
  ...
  const subject = `${name} — your studio listing on FindStudioSpace`
 ...
 * Calls getMailingAddress() which requires assertCanSpamCompliant() to have
 * passed first — call assertCanSpamCompliant() before calling this.
 */
