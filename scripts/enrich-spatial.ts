 ...
 * enrich_listing_spatial() per listing. Populates listing_enrichment with
 * zoning, noise, flood, transit, etc.
 *
 * Run AFTER:
 ...
 *   npx tsx scripts/enrich-spatial.ts --listing-id=42
 *   npx tsx scripts/enrich-spatial.ts --stale=7   (re-enrich if >7 days old)
 *
 * Env:
...
async function enrichRowByRow(staleDays: number): Promise<void> {
  ...
  console.log(`  ${ids.length} listings to enrich`)
  let enriched = 0
  for (const id of ids) {
    const { error: e } = await db.rpc('enrich_listing_spatial', { p_listing_id: id })
    if (e) console.warn(`  listing ${id}: ${e.message}`)
    else enriched++
    if (enriched % 20 === 0) process.stdout.write(`  ${enriched}/${ids.length}…\r`)
  }

  console.log(`\n[enrich-spatial] ✅ enriched ${enriched}/${ids.length}`)
}
